...
export default function TeamPlans() {
    ...
    return <PageWithSubMenu subMenu={getTeamSettingsMenu({ team, showPaymentUI })} title="Plans" subtitle="Manage team plans and billing.">
        ...
        <div className="mt-4 space-x-4 flex">
            <SelectableCard className="w-36 h-32" title="Free" selected={!teamSubscription?.planId} onClick={() => {}}>
                {members.length} x {Currency.getSymbol(currency || 'USD')}0 = {Currency.getSymbol(currency || 'USD')}0
            </SelectableCard>
            {availableTeamPlans.map(tp => <SelectableCard className="w-36 h-32" title={tp.name} selected={tp.chargebeeId === teamSubscription?.planId} onClick={() => checkout(tp.chargebeeId)}>
