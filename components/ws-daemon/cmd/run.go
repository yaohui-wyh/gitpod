...
var runCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if cfg.ReadinessProbeAddr != "" {
			...
			wsManagerFQDN := fmt.Sprintf("ws-manager.%v.svc.cluster.local", os.Getenv("KUBE_NAMESPACE"))
			health.AddReadinessCheck("dns", kubernetes.DNSCanResolveProbe(wsManagerFQDN, 1*time.Second))
